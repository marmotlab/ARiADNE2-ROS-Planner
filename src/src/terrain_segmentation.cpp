...
pcl::PointCloud<pcl::PointXYZI>::Ptr terrainScan(new pcl::PointCloud<pcl::PointXYZI>());
...
int main(int argc, char** argv)
{
  ...
  ros::Publisher pubTerrainCloud = nh.advertise<sensor_msgs::PointCloud2>("/terrain_cloud", 2);
  ...
  while (status)
  {
    ...
    if (newlaserCloud)
    {
      ...
      if (terrainCloudElev->points.empty()) continue;

      pcl::KdTreeFLANN<pcl::PointXYZI> kdtree;
      kdtree.setInputCloud(terrainCloudElev);

      std::vector<int> pointIdxNearest(1);  // Vector to store the index of the nearest neighbor
      std::vector<float> pointNearestSquaredDistance(1);  // Vector to store the squared distance

      terrainScan->clear();
      int terrainScanSize = laserCloudCrop->points.size();
      for (int i = 0; i < terrainScanSize; i++) {
        point = laserCloudCrop->points[i];

        // Perform nearest neighbor search
        if (kdtree.nearestKSearch(point, 1, pointIdxNearest, pointNearestSquaredDistance) > 0)
        {
          if (pointNearestSquaredDistance[0] <= 0.001) {
            pcl::PointXYZI nearest = terrainCloudElev->points[pointIdxNearest[0]];
            terrainScan->push_back(nearest);
          }
        }
      }
      ...
      pcl::toROSMsg(*terrainScan, terrainCloud2);
